...
from .utils import ANSWER_LIST, SHORT_QUESTION_LIST, LONG_QUESTION_LIST,DEFAULT_IMAGE_TOKEN,EXPLANATORY_QUESTION_LIST
...
class SemSegDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i,sampled_cls in enumerate(sampled_classes):
            if ds in ["paco_lvis", "pascal_part"] and self.tuning==True:
...
class SemPartDataset(torch.utils.data.Dataset):
    pixel_mean = torch.Tensor([123.675, 116.28, 103.53]).view(-1, 1, 1)
    pixel_std = torch.Tensor([58.395, 57.12, 57.375]).view(-1, 1, 1)
    img_size = 1024
    ignore_label = 255
...
'''
class ReasonPartDataset(torch.utils.data.Dataset):
    pixel_mean = torch.Tensor([123.675, 116.28, 103.53]).view(-1, 1, 1)
    pixel_std = torch.Tensor([58.395, 57.12, 57.375]).view(-1, 1, 1)
    img_size = 1024
    ignore_label = 255
    ...
    def __init__(
        ...
        self.data2list = {}
        self.data2classes = {}
        #with open('dataset/vlpart/paco/reasonqa.json', 'r') as file:
            #self.paco_description_map = json.load(file)
        with open('dataset/vlpart/pascal_part/reasonqa.json', 'r') as file:
            self.pp_description_map = json.load(file)
    ...
    def __getitem__(self, idx):
        ...
        if ds in ["paco_lvis", "pascal_part"]:
            ...
            description_map = self.pp_description_map
            for ann in sampled_anns:
                sampled_cls = class_map[ann["category_id"]]
                if isinstance(sampled_cls, tuple):
                    obj, part = sampled_cls
        ...
        for i,sampled_cls in enumerate(sampled_classes):
            if ds == "pascal_part":
                text = sampled_description[i]["input"].strip()
                #question_template = random.choice(self.long_question_list)
                questions.append((
                        DEFAULT_IMAGE_TOKEN
                        + "\n"
                ...
                question_template = random.choice(self.short_question_list)
                questions.append(question_template.format(class_name=text.lower()))
            answer_template = '. '.join(sampled_description[i]["COT"].split(". ")[:-1]) + '.'
            answer_template ="{} ".format(answer_template.strip())+ random.choice(self.answer_list) 
            answers.append(answer_template)
