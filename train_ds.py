...
def main(args):
    ...
    if args.auto_resume and len(args.resume) == 0:
        resume = os.path.join(args.log_dir, "ckpt_model/9")
    ...
    '''
    args.resume = os.path.join(args.log_base_dir, "cross2mask0221wovqa/ckpt_model/6")
    load_path, client_state = model_engine.load_checkpoint(args.resume,load_module_strict=False)
    with open(os.path.join(args.resume, "latest"), "r") as f:
        ckpt_dir = f.readlines()[0].strip()
    print("resume training from cross2mask0221wovqa")
    args.start_epoch = 0'''
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.no_eval == False:
            ...
            is_best = giou > best_score #or miou> best_miou or giou2 > best_score2
...
def validate(val_loader, model_engine, epoch, writer, args,partflag):
    intersection_meter = AverageMeter("Intersec", ":6.3f", Summary.SUM)
    intersection_small = AverageMeter("Intersmall", ":6.3f", Summary.SUM)
    intersection_medium = AverageMeter("Intermedium", ":6.3f", Summary.SUM)
    intersection_large = AverageMeter("Interlarge", ":6.3f", Summary.SUM)
    union_meter = AverageMeter("Union", ":6.3f", Summary.SUM)
    union_small = AverageMeter("Union_small", ":6.3f", Summary.SUM)
    union_medium = AverageMeter("Union_medium", ":6.3f", Summary.SUM)
    union_large = AverageMeter("Union_large", ":6.3f", Summary.SUM)
    acc_iou_meter = AverageMeter("gIoU", ":6.3f", Summary.SUM)
    model_engine.eval()
    ...
    for input_dict in tqdm.tqdm(val_loader):
        ...
        for mask_i, output_i in zip(masks_list, output_list):
            ...
            total = mask_i.numel()
            count = torch.sum(mask_i == 1).item()
            ratio = count / total
            if ratio<0.028:
                intersection_small.update(intersection_i.cpu().numpy()), union_small.update(union_i.cpu().numpy())
            elif ratio>0.115:
                intersection_large.update(intersection_i.cpu().numpy()), union_large.update(union_i.cpu().numpy())
            else:
                intersection_medium.update(intersection_i.cpu().numpy()), union_medium.update(union_i.cpu().numpy())
    ...
    intersection_meter.all_reduce()
    intersection_small.all_reduce()
    intersection_medium.all_reduce()
    intersection_large.all_reduce()
    union_meter.all_reduce()
    union_small.all_reduce()
    union_medium.all_reduce()
    union_large.all_reduce()
    acc_iou_meter.all_reduce()

    iou_class = intersection_meter.sum / (union_meter.sum + 1e-10)
    iou_small = intersection_small.sum / (union_small.sum + 1e-10)
    iou_medium = intersection_medium.sum / (union_medium.sum + 1e-10)
    iou_large = intersection_large.sum / (union_large.sum + 1e-10)
    ciou = iou_class[1]
    ciou_s,ciou_m,ciou_l = iou_small[1],iou_medium[1],iou_large[1]
    giou = acc_iou_meter.avg[1]

    #if args.local_rank == 0:
    writer.add_scalar("val/giou", giou, epoch)
    writer.add_scalar("val/ciou", ciou, epoch)
    print("giou: {:.4f}, ciou: {:.4f},ciou_s: {:.4f},ciou_m: {:.4f},ciou_l: {:.4f}".format(giou, ciou,ciou_s,ciou_m,ciou_l))

    return giou, ciou

def validate2(val_loader2, model_engine, epoch, writer, args):
    intersection_meter2 = AverageMeter("Intersec", ":6.3f", Summary.SUM)
    union_meter2 = AverageMeter("Union", ":6.3f", Summary.SUM)
    ...
    for input_dict in tqdm.tqdm(val_loader2):
        ...
        intersection_meter2.update(intersection), union_meter2.update(
            union
        ),miou_meter.update(miou,n=masks_list.shape[0])


    intersection_meter2.all_reduce()
    union_meter2.all_reduce()
