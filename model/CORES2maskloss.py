...
class CORESForCausalLM(LlavaLlamaForCausalLM):
    ...
    def model_forward(
        ...
        **kwargs,
        ...
        cv2.imwrite('sema_gt/'+kwargs["image_paths"][0].split('/')[-1], image)
        for p,gt_i in enumerate(masks_list[0]):
                ...
                tmp_dir = 'sema_gt/'+kwargs["image_paths"][0].split('/')[-1].split('.')[0]+'_gt'+str(p)
        ...
        for i in range(len(masks_list)):
            ...
            new_size = 64
            for m in range(masks_list[i].shape[0]):
                ...
                padded_mask = F.pad(masks_list[i][m].unsqueeze(0).unsqueeze(0), (0, pad_width, 0, pad_height), mode='constant', value=0)

                # Resize the padded image
                resized_mask = F.interpolate(padded_mask, size=(new_size, new_size), mode='bilinear').squeeze()

                # Flatten the masks tensor
                flat_mask = resized_mask.view(-1)
                # Randomly sample 3 points with value 1 and 2 points with value 0
                ones_indices = (resized_mask == resized_mask.max()).nonzero()
                zeros_indices = (resized_mask == 0).nonzero()
                if len(ones_indices)!=0:
                    sampled_ones = ones_indices[torch.randperm(len(ones_indices))[:1]]
                    sampled_zeros = zeros_indices[torch.randperm(len(zeros_indices))[:1]]
                else:
                    sampled_ones = ones_indices[torch.randperm(len(ones_indices))[:0]]
                    sampled_zeros = zeros_indices[torch.randperm(len(zeros_indices))[:2]]                    
                # Extract coordinates for sampled points
                coords = torch.cat((sampled_ones, sampled_zeros))
                # Create the output tensor containing the coordinates
                #coords_tensor = torch.stack((coords // new_size, coords % new_size)).t()
                # Create the output tensor containing the values of the points
                values_tensor = torch.tensor(sampled_ones.shape[0]*[resized_mask.max()]+sampled_zeros.shape[0]*[0],dtype=pred_embeddings[i].dtype).cuda()
                # Create the tuple format output for each mask
                coordss.append(coords)
                valuess.append(values_tensor)
            try:
                results = (torch.stack(coordss, dim=0), torch.stack(valuess, dim=0))
            ...
            for p,low in enumerate(pred_mask[:, 0]):
                ...
                heatmap = cv2.applyColorMap(result22[0], cv2.COLORMAP_JET)
                result2 = cv2.addWeighted(image,0.7,heatmap,0.3,0)
                tmp_dir = 'sema_gt/'+kwargs["image_paths"][0].split('/')[-1].split('.')[0]+'_dt'+str(p)
                for rr in result22:
                    while os.path.exists(tmp_dir+'_sema.jpg'):
                        tmp_dir += '_0'                    
                    cv2.imwrite(tmp_dir+'_sema.jpg', rr)
                    cv2.imwrite(tmp_dir+'_sema_merge.jpg', result2)'''
        ...
        for i in range(len(pred_sema_embeddings)):            
            ...
            pred_sema_mask = self.model.visual_model.postprocess_masks(
                low_res_masks,
                input_size=resize_list[i],
                original_size=label_list[i].shape,
            )
            pred_sema_masks.append(pred_sema_mask[:, 0])
            '''for p,low in enumerate(low_res_masks):
                ...
                tmp_dir = 'coresimg/'+kwargs["image_paths"][0].split('/')[-1].split('.')[0]+'_dt'+str(2*p)
                while os.path.exists(tmp_dir+'.jpg'):
                    tmp_dir += '_0'                    
            ...
            '''for p,low in enumerate(low_res_masks):
                ...
                heatmap = cv2.applyColorMap(result22[0][0], cv2.COLORMAP_JET)
                result2 = cv2.addWeighted(img.to(dtype=float).detach().cpu().numpy().astype(np.uint8),0.7,heatmap,0.3,0)
                tmp_dir = 'coresimg/'+kwargs["image_paths"][0].split('/')[-1].split('.')[0]+'_dt'+str(2*p+1)
                while os.path.exists(tmp_dir+'.jpg'):
                    tmp_dir += '_0'                    
                cv2.imwrite(tmp_dir+'.jpg', result22[0][0])
                cv2.imwrite(tmp_dir+'_merge.jpg', result2)'''
        ...
        for batch_idx in range(len(pred_masks)):
            ...
            pred_sema_mask = pred_sema_masks[batch_idx]
            ...
            if gt_sema_mask.shape[0] == pred_sema_mask.shape[0]:
                mask_bce_loss += sigmoid_ce_loss(pred_sema_mask, gt_sema_mask, num_masks=gt_sema_mask.shape[0])* gt_sema_mask.shape[0]
                mask_dice_loss += dice_loss(pred_sema_mask, gt_sema_mask, num_masks=gt_sema_mask.shape[0])* gt_sema_mask.shape[0]
