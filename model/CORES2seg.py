...
class CORESForCausalLM(LlavaLlamaForCausalLM):
    ...
    def model_forward(
        ...
        **kwargs,
        ...
        mask_bce_loss1 = self.bce_loss_weight * mask_bce_loss1 / (num_masks + 1e-8)
        mask_dice_loss1 = self.dice_loss_weight * mask_dice_loss1 / (num_masks + 1e-8)
        ###############2seg#######################
        ...
        pred_notempty = [i.detach() for i in low_mask_list if i.shape[0]!=0]
        masked_feat = Weighted_GAP(image_embeddings)
        if pred_notempty!=[]:
            target_sizes = [max(mask.shape[1], mask.shape[2]) for mask in pred_notempty]  # 计算最大的尺寸

            for target_size,mask in zip(target_sizes,pred_notempty):
                #if mask.shape[0]==0:
                    #continue
                ...
                '''pad_h = max(0, target_size - mask.shape[-2])
                pad_w = max(0, target_size - mask.shape[-1])
                # 在高度和宽度两个维度上进行填充
                padded_mask = F.pad(normalized_tensor, (0, pad_w, 0, pad_h), value=0)'''
                ...
                #image_embeddings2[start:end] = F.interpolate(normalized_tensor, size=(64, 64), mode='bilinear', align_corners=False).expand(-1,256,-1,-1).to(dtype=torch.bfloat16)*image_embeddings2[start:end].unsqueeze(0).expand(mask.shape()[0],-1,-1,-1)
            ...
            embedding_cat = self.transformer_cross_attention_layers(torch.cat(pred_embeddings,dim=0).unsqueeze(0),masked_feat.flatten(2).permute(2,0,1)).permute(1,2,0).squeeze(2)
            for i in range(len(seg_token_offset) - 1):
                start_i, end_i = seg_token_offset[i], seg_token_offset[i + 1]
